...
function onFail(ruleNames, vocabulary, state, tokens, collector, parserStack, alreadyPassed, 
  ...
  const failureRules = options.recovery;
  const currentRule = parserStack[parserStack.length-1][0];
  if (Array.isArray(failureRules)) {
    for (const rule of failureRules) {
        const {ifInRule, nested, andFindToken, thenGoToRule} = rule;
        const a = nested ? isInside(ifInRule, parserStack) : {remainingStack: parserStack,  result: ifInRule === currentRule}
        if (a.result) {
          for (let i = tokenStreamIndex; i < tokens.length; i++) {
            if (tokens[i].type === andFindToken) {
              process(ruleNames, vocabulary, 
                ...
                tokens, collector, a.remainingStack, alreadyPassed, i, atn, options);
              }
            }
        }
        return;
  ...
  if (failureRules[currentRule]) {
    for (let i = tokenStreamIndex; i < tokens.length; i++) {
      if (failureRules[currentRule][tokens[i].type]) {
        process(ruleNames, vocabulary, atn.ruleToStartState[failureRules[currentRule][tokens[i].type]], tokens, collector, parserStack, alreadyPassed, i, atn, options)
      }
    }
    console.log("Estamos ahí");
  }
}
...
function process(ruleNames, vocabulary, state, tokens, collector, parserStack, alreadyPassed, 
  ...
  const tryToRecover = () => onFail(ruleNames, vocabulary, state, tokens, collector, parserStack, alreadyPassed, 
    tokenStreamIndex, atn, options);
