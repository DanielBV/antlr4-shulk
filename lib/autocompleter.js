...
class Suggestion {
  ...
  polish(lexer, ruleNames) {
    console.log(this.suggestion);
    const x = this._raw;
    if (!this.isRule) this.suggestion = x === -1 ? "EOF" : lexer.symbolicNames[x] || lexer.literalNames[x].slice(1, -1);
  }
}
...
export default class Autocompleter {
    ...
    autocomplete(input) {
        ...
        grouped.forEach(x => x.polish(this._lexer));
